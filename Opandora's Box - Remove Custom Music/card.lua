...
function Card:set_sprites(_center, _front)
    ...
    if _center then 
        if _center.set then
            ...
            if _center.name == 'Crumpled Lucky Card' and (_center.discovered or self.bypass_discovery_center) then 
                self.children.center.scale.y = self.children.center.scale.x
            end

            if _center.name == 'Last Resort' and (_center.discovered or self.bypass_discovery_center) then 
                self.children.center.scale.y = self.children.center.scale.y/1.2
            end
...
function Card:set_ability(center, initial, delay_sprites)
	...
	if center.name == "Last Resort" and (center.discovered or self.bypass_discovery_center) then 
		H = H/1.2
		self.T.h = H
	end
    ...
    if center.name == "Crumpled Lucky Card" and (center.discovered or self.bypass_discovery_center) then 
        H = W
        self.T.h = H
    end
    ...
    self.ability = {
        ...
        x_chip = center.config.Xchip or 1,
    ...
    if self.ability.name == 'Chameleon Hook' then
        local _poker_hands = {}
        for k, v in pairs(G.GAME.hands) do
            if v.visible then _poker_hands[#_poker_hands+1] = k end
        end
        local old_hand = self.ability.hook_poker_hand
        self.ability.hook_poker_hand = nil
        local old_hand = self.ability.hook_poker_hand
        self.ability.hook_poker_hand = nil
        while not self.ability.hook_poker_hand do
            self.ability.hook_poker_hand = pseudorandom_element(_poker_hands, pseudoseed((self.area and self.area.config.type == 'title') and 'false_to_do' or 'to_do'))
            if self.ability.hook_poker_hand == old_hand then self.ability.hook_poker_hand = nil end
        end
    end
...
function Card:generate_UIBox_ability_table()
    ...
    elseif self.ability.set == 'Joker' then -- all remaining jokers
		...
		--CUSTOM_JOKERS_BELOW-- --CUSTOM_JOKERS_BELOW-- --CUSTOM_JOKERS_BELOW-- --CUSTOM_JOKERS_BELOW-- 
		elseif self.ability.name == 'Cobblestone Joker' then loc_vars = {self.ability.extra, self.ability.mult}
		elseif self.ability.name == 'Ooga Booga' then loc_vars = {self.ability.extra, self.ability.x_mult}
		elseif self.ability.name == 'Miner' then loc_vars = {self.ability.extra+1}
		elseif self.ability.name == 'Crumpled Lucky Card' then loc_vars = {''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds, self.ability.extra.Xmult}
		elseif self.ability.name == 'Croon' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Baroness' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'By The Nine!' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Litany' then loc_vars = {self.ability.extra, self.ability.x_mult}
		elseif self.ability.name == 'Lifeline' then
		elseif self.ability.name == 'Unum' then loc_vars = {self.ability.extra+1}
		elseif self.ability.name == 'Inferno' then loc_vars = {self.ability.extra.Xmult}
		elseif self.ability.name == 'Void' then loc_vars = {localize{type = 'name_text', set = 'Tag', key = 'tag_negative', nodes = {}}}
		elseif self.ability.name == 'Disciple' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Encore' then
		elseif self.ability.name == 'Mafia' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Cracked Egg' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Blob' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Moolah' then loc_vars = {self.ability.extra, self.ability.extra * math.floor((self.ability.gold_tally or 0) / 2)}
		elseif self.ability.name == 'Pyramid' then loc_vars = {self.ability.extra, self.ability.x_mult}
		elseif self.ability.name == 'Black Friday' then loc_vars = {localize{type = 'name_text', set = 'Tag', key = 'tag_coupon', nodes = {}}}
		elseif self.ability.name == 'Framework' then loc_vars = {''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == 'Hoarder' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Last Resort' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Potato Chips' then loc_vars = {self.ability.extra.chips, self.ability.extra.chip_mod}
		elseif self.ability.name == 'Number Two' then loc_vars = {self.ability.extra, localize('Flush', 'poker_hands'), self.ability.mult}
		elseif self.ability.name == 'Chromahand' then loc_vars = {''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra}
		elseif self.ability.name == "No, Thank You" then loc_vars = {self.ability.extra, self.ability.x_mult}
		elseif self.ability.name == "Oh, Lord" then
		elseif self.ability.name == 'Ace In The Hole' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Chameleon Hook' then loc_vars = {localize(self.ability.hook_poker_hand, 'poker_hands')}
		elseif self.ability.name == 'Gasbag' then loc_vars = {self.ability.extra.Xmult, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == "Fermata's Tremolo" then loc_vars = {self.ability.extra.plus, self.ability.extra.total}
		elseif self.ability.name == 'Balanzone' then
		elseif self.ability.name == 'Scratch Card' then loc_vars = {''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds1, self.ability.extra.odds2, self.ability.extra.dollars}
		elseif self.ability.name == 'Cicero' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Leprechaun Joker' then loc_vars = {self.ability.extra+1}
		elseif self.ability.name == 'Pocket Lint' then loc_vars = {self.ability.extra.Xchip, self.ability.extra.basechips}
		elseif self.ability.name == 'More The Merrier' then loc_vars = {self.ability.extra, (G.jokers and G.jokers.cards and #G.jokers.cards or 0)*self.ability.extra}
		elseif self.ability.name == 'Mr. Hands' then loc_vars = {self.ability.extra, self.ability.extra*(G.GAME.hands_played and G.GAME.hands_played or 0)}
		elseif self.ability.name == 'Castor' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Pollux' then loc_vars = {self.ability.extra}
		elseif self.ability.name == 'Russian Roulette' then loc_vars = {''..(G.GAME and G.GAME.probabilities.normal) or 1, self.ability.extra.odds}
		
		elseif self.ability.name == 'Pendulum' then 
		loc_vars = {self.ability.extra.Xmult, self.ability.extra.Xchips, 
		localize{type = 'variable', key = (self.ability.extra.toggle == 1) and 'currently_mult' or 'currently_chip'}}
		
		elseif self.ability.name == 'Sacrificial Tanto' then loc_vars = {self.ability.extra, self.ability.x_mult}
		elseif self.ability.name == 'Spirit Box' then
		elseif self.ability.name == 'White Collar Joker' then loc_vars = {self.ability.extra, self.ability.x_chip}
		elseif self.ability.name == 'Multi-Tool' then loc_vars = {self.ability.extra.chip_mod, self.ability.extra.mult, self.ability.extra.chips, self.ability.mult}
		elseif self.ability.name == 'Eye for an Eye' then loc_vars = {self.ability.extra, self.ability.x_chip}
		elseif self.ability.name == 'Mezzetino' then loc_vars = {self.ability.extra.gain, self.ability.extra.total}
		elseif self.ability.name == 'Garlic' then loc_vars = {self.ability.extra.chip, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == 'Red Pepper Flakes' then loc_vars = {self.ability.extra.mult, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == 'Olive Oil' then loc_vars = {self.ability.extra.chip, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == 'Pasta' then loc_vars = {self.ability.extra.mult, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds}
		elseif self.ability.name == 'Aglio e Olio' then 
		loc_vars = {self.ability.extra.Xchip, self.ability.extra.Xmult, ''..(G.GAME and G.GAME.probabilities.normal or 1), self.ability.extra.odds, self.ability.x_chip, self.ability.x_mult}
		elseif self.ability.name == 'Tab' then loc_vars = {self.ability.extra.gain, self.ability.extra.lose}
		elseif self.ability.name == 'Polyglot Joker' then
			loc_vars = {
				self.ability.extra.dollars,
				self.ability.extra.mult,
				self.ability.extra.chips,
				self.ability.extra.Xmult,
				self.ability.extra.Xchips,
				localize{
					type = 'variable',
					key = ({
						[1] = 'polyglot_dollars',
...
function Card:use_consumeable(area, copier)
    ...
    if self.ability.consumeable.mod_conv or self.ability.consumeable.suit_conv then
		...
		elseif self.ability.name == 'Fragility' then
			for i=1, #G.hand.highlighted do
				G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.1, func = function()
					local card = G.hand.highlighted[i]
					local suit_prefix = string.sub(card.base.suit, 1, 1)..'_' 
	...
	if self.ability.name == 'Summon' then
		if G.GAME.last_spectral == 'c_soul' or G.GAME.last_spectral == 'c_black_hole' then
			--if last used spectral was a soul or a black hole, then don't. but the use button is still usable lol. idk how to code it to make it not. 
			G.E_MANAGER:add_event(Event({
				trigger = 'after',
				delay = 0.4,
				func = function()
				play_sound('didyoutrytosummonanothersoulorblackholelmaonicetry') --plays the ERRR WRONG buzzer sfx
				return true end}))
			card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize('k_nosummon'), colour = G.C.RED})
		else
			--else, summon last used spectral
			G.E_MANAGER:add_event(Event({
				trigger = 'after',
				delay = 0.4,
				func = function()
					if G.consumeables.config.card_limit > #G.consumeables.cards then
						play_sound('timpani')
						local card = create_card('Spectral', G.consumeables, nil, nil, nil, nil, G.GAME.last_spectral, 'summon')
						card:add_to_deck()
						G.consumeables:emplace(card)
						used_tarot:juice_up(0.3, 0.5)
					end
        ...
        if self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex' or pseudorandom('wheel_of_fortune') < G.GAME.probabilities.normal/self.ability.extra then 
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
                ...
                if self.ability.name == 'Hex' then 
                    local _first_dissolve = nil
                    for k, v in pairs(G.jokers.cards) do
                        if v ~= eligible_card and (not v.ability.eternal) then v:start_dissolve(nil, _first_dissolve);_first_dissolve = false end
                    end
                end
                if self.ability.name == 'Ectoplasm' then 
                    G.GAME.ecto_minus = G.GAME.ecto_minus or 0
...
function Card:can_use_consumeable(any_state, skip_check)
    ...
    if G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT or any_state then
        ...
        if self.ability.name == 'Summon' then
            if (#G.consumeables.cards < G.consumeables.config.card_limit or self.area == G.consumeables) 
                and G.GAME.last_spectral and G.GAME.last_spectral~= 'c_summon' then return true end
        end
...
function Card:calculate_dollar_bonus()
    if self.debuff then return end
    if self.ability.set == "Joker" then
		if self.ability.name == 'Moolah' and self.ability.gold_tally and self.ability.gold_tally > 0 then
			return self.ability.extra * math.floor(self.ability.gold_tally / 2)
		end
		if self.ability.name == 'Mezzetino' then
			self.ability.extra.total = 0
			for i = 1, #G.jokers.cards do
				if G.jokers.cards[i] ~= self and G.jokers.cards[i].ability.set == 'Joker' then
					self.ability.extra.total = self.ability.extra.total + (G.jokers.cards[i].sell_cost * self.ability.extra.gain)
				end
			end
			return self.ability.extra.total
		end
        if self.ability.name == 'Tab' then
            return self.ability.extra.gain
        end
...
function Card:calculate_joker(context)
    ...
    if self.ability.set == "Joker" and not self.debuff then
		...
		if self.ability.name == "Balanzone" then
			context.blueprint = (context.blueprint and (context.blueprint + 1)) or 1
			context.blueprint_card = context.blueprint_card or self
			if context.blueprint > #G.jokers.cards + 1 then return end
        ...
        elseif context.selling_self then
			if self.ability.name == 'Spirit Box' then
				local still_has_spirit_box = false
				for _, joker in ipairs(G.jokers.cards) do
					if joker ~= self and joker.ability.name == 'Spirit Box' then
						still_has_spirit_box = true
						break
					end
				end
				G.GAME.spectral_rate = still_has_spirit_box and 2 or 0
			end
			if self.ability.name == 'Mezzetino' and not context.blueprint then
				change_shop_size(1)
			end
			if self.ability.name == 'Scratch Card' then
				self.sell_cost = 0
				if pseudorandom('odds1') < G.GAME.probabilities.normal/self.ability.extra.odds1 then
				G.E_MANAGER:add_event(Event({func = function() 
					local card = create_card(card_type, G.consumeables, nil, nil, nil, nil, 'c_soul')
					card:add_to_deck()
					G.consumeables:emplace(card)
					play_sound('jackpothit')
				return true end}))   
			card_eval_status_text(context.blueprint_card or self, 'extra', nil, nil, nil, {message = localize('k_lotterysoul'), colour = G.C.PURPLE})
			elseif pseudorandom('odds2') < G.GAME.probabilities.normal/self.ability.extra.odds2 then
			ease_dollars(self.ability.extra.dollars)
			...
			play_sound('jackpothit')
			return true end}))
			card_eval_status_text(context.blueprint_card or self, 'extra', nil, nil, nil, {message = localize('k_lotterysoul'), colour = G.C.MONEY})
				return {
					dollars = self.ability.extra.dollars,
						}
				end
			end
			if self.ability.name == 'Lifeline' then
			G.E_MANAGER:add_event(Event({
				func = (function()
			...
			if self.ability.name == 'Cracked Egg' then
					self.sell_cost = 0
				end
            if self.ability.name == 'Void' then
                G.E_MANAGER:add_event(Event({
                    func = (function()
        ...
        elseif context.reroll_shop then
			if self.ability.name == 'Spirit Box' and not context.blueprint then
				local has_spirit_box = false
				for _, joker in ipairs(G.jokers.cards) do
					if joker.ability.name == 'Spirit Box' then
						has_spirit_box = true
						break
					end
				end
				G.GAME.spectral_rate = has_spirit_box and 2 or 0
			end
        ...
        elseif context.ending_shop then
				...
				--don't judge this spaghetti code too harsh my brian small
				--originally, this Joker activates at the end of round. It did not work. AT ALL. Gave me fucking infinite Fool cards. Now it's here because I give up LMAO.
				if self.ability.name == 'Encore' and not context.blueprint then
				G.E_MANAGER:add_event(Event({func = function() 
						local card = create_card(card_type, G.consumeables, nil, nil, nil, nil, 'c_fool')
						card:set_edition({negative = true}, true)
						card:add_to_deck()
						G.consumeables:emplace(card)
						play_sound('encoresfx')
					return true end}))   
				card_eval_status_text(context.blueprint_card or self, 'extra', nil, nil, nil, {message = localize('k_encore'), colour = G.C.PURPLE})
			else
				if self.ability.name == 'Encore' then
				local blueprint_multiplier = context.blueprint_card and 1 or 0  --+1 if copied by Blueprint, +0 otherwise
				G.E_MANAGER:add_event(Event({func = function() 
					for i = 1, blueprint_multiplier do  
						local card = create_card(card_type, G.consumeables, nil, nil, nil, nil, 'c_fool')
						card:set_edition({negative = true}, true)
						card:add_to_deck()
						G.consumeables:emplace(card)
        ...
        elseif context.first_hand_drawn then
            ...
            if self.ability.name == 'Mafia' and not context.blueprint then
        ...
        elseif context.setting_blind and not self.getting_sliced then
			...
			if self.ability.name == 'Blob' and not (context.blueprint_card or self).getting_sliced then
				G.E_MANAGER:add_event(Event({func = function()
					G.GAME.current_round.hands_left = 1
					play_sound('card1')
					card_eval_status_text(context.blueprint_card or self, 'extra', nil, nil, nil, {message = localize('k_blob'), colour = G.C.BLUE}) 
				return true end }))
			end
			if self.ability.name == 'Russian Roulette'
			and pseudorandom('rusroul') < G.GAME.probabilities.normal/self.ability.extra.odds
			and not (context.blueprint_card or self).getting_sliced then
				--local for jokers that can be destroyed, like Madness
				local destructable_jokers = {}
				for i = 1, #G.jokers.cards do
					if G.jokers.cards[i] ~= self and not G.jokers.cards[i].ability.eternal and not G.jokers.cards[i].getting_sliced then 
				...
				if random_joker and random_joker ~= self then 
					if pseudorandom('rusroul_fifty_fifty') < 0.5 then --rusroul_fifty_fifty because it's a 50/50 to give Negative or get destroyed
				...
				if self.ability.name == 'Sacrificial Tanto' and not context.blueprint then --i just copied ceremonial dagger's code sorry!!
					local my_pos = nil
					for i = 1, #G.jokers.cards do
					...
					if my_pos and G.jokers.cards[my_pos-1] and not self.getting_sliced and not G.jokers.cards[my_pos-1].ability.eternal and not G.jokers.cards[my_pos-1].getting_sliced then 
						...
						G.E_MANAGER:add_event(Event({func = function()
							...
							return true end }))
						card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_xmult', vars = {self.ability.x_mult + self.ability.extra}}, colour = G.C.RED, no_juice = true})
					end
				end
				if self.ability.name == 'White Collar Joker' and context.blind.boss and not context.blueprint then
                self.ability.x_chip = self.ability.x_chip + self.ability.extra
                if not (context.blueprint_card or self).getting_sliced then
                    card_eval_status_text((context.blueprint_card or self), 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_xchip', vars = {self.ability.x_chip}}})
                end
            end
			if self.ability.name == 'Polyglot Joker' and not context.blueprint then
				local current_effect = self.ability.extra.toggle
				...
				self.ability.extra.toggle = new_effect
				local messages = {
				...
				card_eval_status_text(context.blueprint_card or self, 'extra', nil, nil, nil, messages[new_effect])
			end
        ...
        elseif context.destroying_card and not context.blueprint then
			...
			if self.ability.name == 'Inferno' and #context.full_hand > 0 then
                return {
                }
            end
            return nil
        elseif context.cards_destroyed then
			if self.ability.name == 'Multi-Tool' and not context.blueprint then
				local faces = 0
				...
				if faces > 0 then
					G.E_MANAGER:add_event(Event({func = function()
						self.ability.extra.chips = self.ability.extra.chips + faces*self.ability.extra.chip_mod
						self.ability.mult = self.ability.mult + faces * self.ability.extra.mult
						card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_chips', vars = {self.ability.extra.chips}}.. ' Chips' })
						G.E_MANAGER:add_event(Event({func = function() return true end, delay = 0.3}))
						G.E_MANAGER:add_event(Event({func = function()
							card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_mult', vars = {self.ability.mult}}})
							return true end}))
						return true end}))
					end
				end
				if self.ability.name == 'Eye for an Eye' and not context.blueprint then
					local bonuscards = 0
					for k, v in ipairs(context.glass_shattered) do
					...
					if bonuscards > 0 then
						G.E_MANAGER:add_event(Event({func = function()
						G.E_MANAGER:add_event(Event({func = function()
							self.ability.x_chip = self.ability.x_chip + bonuscards*self.ability.extra
							return true end}))
						card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_xchip', vars = {self.ability.x_chip + bonuscards*self.ability.extra}}})
						return true end}))
					end
				end
        ...
        elseif context.remove_playing_cards then
			if self.ability.name == 'Multi-Tool' and not context.blueprint then
				...
				if face_cards > 0 then
					self.ability.extra.chips = self.ability.extra.chips + face_cards*self.ability.extra.chip_mod
					self.ability.mult = self.ability.mult + face_cards * self.ability.extra.mult
					G.E_MANAGER:add_event(Event({func = function()
						card_eval_status_text(self, 'extra', nil, nil, nil, {message = localize{type = 'variable', key = 'a_chips', vars = {self.ability.extra.chips}}.. ' Chips'})
						G.E_MANAGER:add_event(Event({func = function() return true end, delay = 0.3}))
						G.E_MANAGER:add_event(Event({func = function()
        ...
        elseif context.using_consumeable then
            ...
            if self.ability.name == 'Litany' and not context.blueprint and context.consumeable.ability.set == 'Spectral' then
                self.ability.x_mult = self.ability.x_mult + self.ability.extra
                G.E_MANAGER:add_event(Event({func = function() 
					card_eval_status_text(self, 'extra', nil, nil, nil, 
					{message = localize{type='variable',key='a_xmult',
					vars={self.ability.x_mult}}})
					; return true end}))
                return
            end
        ...
        elseif context.discard then
			if self.ability.name == 'Ace In The Hole' and not context.blueprint and 
			...
			if self.ability.name == "No, Thank You" and 
        ...
        elseif context.end_of_round then
            ...
            elseif not context.blueprint then
				if self.ability.name == 'Framework' then
					local roll = pseudorandom('framework') --creating local for odds that would hit
					local threshold = G.GAME.probabilities.normal/self.ability.extra.odds --threshold for the actual 1 in ~
				...
				if self.ability.name == 'Polyglot Joker' and not context.blueprint then
					local effect = self.ability.extra.toggle
					if effect == 1 then
						ease_dollars(self.ability.extra.dollars)
						return {
                ...
                if self.ability.name == 'Chameleon Hook' and not context.blueprint then
                    local _poker_hands = {}
                    for k, v in pairs(G.GAME.hands) do
                        if v.visible and k ~= self.ability.hook_poker_hand then _poker_hands[#_poker_hands+1] = k end
                    end
                    self.ability.hook_poker_hand = pseudorandom_element(_poker_hands, pseudoseed('hook'))
                    return {
                        message = localize('k_reset')
                    }
                end
				if self.ability.name == 'Gasbag' then
					if pseudorandom('gasbag') < G.GAME.probabilities.normal / self.ability.extra.odds then
							G.E_MANAGER:add_event(Event({func = function()
								play_sound('tarot1')
								self.T.r = -0.2
								self:juice_up(0.3, 0.4)
							...
							G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.3, blockable = false, func = function() --destroys itself
								...
								return true end })) 
								return true end })) 
								return { message = localize('k_gasbag_dead') }
							else
						return { message = localize('k_gasbag_safe') }
					end
				end
				if self.ability.name == 'Garlic' or 
				   self.ability.name == 'Red Pepper Flakes' or 
				   self.ability.name == 'Olive Oil' or 
				   self.ability.name == 'Pasta' or 
				   self.ability.name == 'Aglio e Olio' then
					if pseudorandom(self.ability.name == 'Aglio e Olio' and 'aglio_e_olio' or 'ingredients') < G.GAME.probabilities.normal/self.ability.extra.odds then 
						G.E_MANAGER:add_event(Event({
							func = function()
								play_sound('tarot1')
								self.T.r = -0.2
								...
								G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.3, blockable = false,
									func = function()
										...
										return true; end})) 
								return true end})) 
						if self.ability.name == 'Garlic' then G.GAME.pool_flags.garlic_extinct = true end
						if self.ability.name == 'Red Pepper Flakes' then G.GAME.pool_flags.red_pepper_flakes_extinct = true end
						if self.ability.name == 'Olive Oil' then G.GAME.pool_flags.olive_oil_extinct = true end
						if self.ability.name == 'Pasta' then G.GAME.pool_flags.pasta_extinct = true end
						if G.GAME.pool_flags.garlic_extinct and 
						   G.GAME.pool_flags.red_pepper_flakes_extinct and 
						   G.GAME.pool_flags.olive_oil_extinct and 
						   G.GAME.pool_flags.pasta_extinct then
						   G.GAME.pool_flags.aglio_e_olio_unlocked = true
						end
						return {
							message = localize('k_ing_added')
						}
					else
						return {
        ...
        elseif context.individual then
            if context.cardarea == G.play then
					if self.ability.name == "Oh, Lord" and #G.consumeables.cards + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
				...
				if self.ability.name == 'Cobblestone Joker' and
					context.other_card.ability.name == 'Stone Card' and not context.blueprint then
						self.ability.mult = self.ability.mult + self.ability.extra
						return {
				...
				if self.ability.name == 'Ooga Booga' and context.other_card.ability.name == 'Stone Card' and not context.blueprint then
					self.ability.x_mult = self.ability.x_mult + self.ability.extra
					G.E_MANAGER:add_event(Event({func = function()
						play_sound('oogabooga')
                        return true end }))
						return {
							extra = {focus = self, message = localize('k_ooga'), colour = G.C.MULT},
							card = self
					}
				end
				if self.ability.name == 'Potato Chips' and
					context.other_card.ability.name == 'Bonus' and not context.blueprint then
						self.ability.extra.chips = self.ability.extra.chips + self.ability.extra.chip_mod
						return {
							extra = {focus = self, message = localize('k_upgrade_ex')},
							colour = G.C.CHIPS,
                ...
                if self.ability.name == 'Blob' then
                        G.GAME.dollar_buffer = (G.GAME.dollar_buffer or 0) + self.ability.extra
                        G.E_MANAGER:add_event(Event({func = (function() G.GAME.dollar_buffer = 0; return true end)}))
                        return {
                            dollars = self.ability.extra,
				...
				if self.ability.name == 'Disciple' and
					context.other_card.ability.name == 'Bonus' then
						return {
							x_chip = self.ability.extra,
							card = self
						}
					end
				if self.ability.name == 'Croon' then
					local chord_sounds = {
						[3] = 'chordiii',
						[4] = 'chordiv',
						[5] = 'chordv'
                ...
                if self.ability.name == 'Baroness' and
                ...
                if self.ability.name == 'By The Nine!' and
				...
				if self.ability.name == 'Inferno' then
					return {
						x_mult = self.ability.extra.Xmult,
						card = self
					}
				end
				if self.ability.name == 'Garlic' and context.other_card:is_suit("Spades") then
					return {
						chips = self.ability.extra.chip,
						card = self
					}
				end
				if self.ability.name == 'Red Pepper Flakes' and context.other_card:is_suit("Hearts") then
					return {
						mult = self.ability.extra.mult,
						card = self
					}
				end
				if self.ability.name == 'Olive Oil' and context.other_card:is_suit("Clubs") then
					return {
						chips = self.ability.extra.chip,
						card = self
					}
				end
				if self.ability.name == 'Pasta' and context.other_card:is_suit("Diamonds") then
					return {
						mult = self.ability.extra.mult,
						card = self
					}
				end
				if self.ability.name == 'Aglio e Olio' then
				self.ability.x_chip = self.ability.extra.Xchip + self.ability.x_chip
				self.ability.x_mult = self.ability.extra.Xmult + self.ability.x_mult
					return {
						extra = {focus = self, message = localize('k_upgrade_ex')},
						card = self
					}
        ...
        elseif context.repetition then
            if context.cardarea == G.play then
				if self.ability.name == "Fermata's Tremolo" then
					return {
						message = localize('k_again_ex'),
						repetitions = self.ability.extra.total,
						card = self
					}
				end
                if self.ability.name == 'Leprechaun Joker' and context.other_card.ability.name == 'Lucky Card' then
                    return {
                        message = localize('k_again_ex'),
                        repetitions = self.ability.extra,
                        card = self
                ...
                if self.ability.name == 'Unum' and (context.other_card:get_id() == 14) then
                    return {
                        message = localize('k_again_ex'),
                        repetitions = self.ability.extra,
                        card = self
                ...
                if self.ability.name == 'Miner' and context.other_card.ability.name == 'Stone Card' then
					G.E_MANAGER:add_event(Event({func = function()
					play_sound('miner')
					return true end }))
						return {
							message = localize('k_miner'),
							repetitions = self.ability.extra,
							card = self
						}
				end
        ...
        elseif context.other_joker then
			if self.ability.name == 'More The Merrier' and context.other_joker.config.center.rarity == 1 then
				G.E_MANAGER:add_event(Event({func = function()
					context.other_joker:juice_up(0.5, 0.5)
					return true end })) 
					return {
					message = localize{type='variable',key='a_xchip',vars={self.ability.extra}},
					Xchip_mod = self.ability.extra
					}
				end
			...
			then
			if self.ability.name == 'Crumpled Lucky Card' and pseudorandom('crumple') < G.GAME.probabilities.normal/self.ability.extra.odds then
				G.E_MANAGER:add_event(Event({func = function()
					play_sound('jackpothit')
					return true end }))
						return {
				...
				if self.ability.name == 'Chromahand' and not context.blueprint then
				local polychrome_cards = {} --table for cards that can turn polychrome
					for i = 1, #context.scoring_hand do
						if pseudorandom('chromahand_' .. i) < G.GAME.probabilities.normal / self.ability.extra then
							if  context.scoring_hand[i].ability and context.scoring_hand[i].ability.name == 'Stone Card' or
								context.scoring_hand[i]:is_suit('Clubs') or
								context.scoring_hand[i]:is_suit('Diamonds') or
								context.scoring_hand[i]:is_suit('Hearts') or
								context.scoring_hand[i]:is_suit('Spades') 
							then
								table.insert(polychrome_cards, context.scoring_hand[i]) --stores the cards into the table, not turned polychrome yet
							end
						end
					end
					if #polychrome_cards > 0 then --will ONLY run the effect if at least one card will turn polychrome, stonks
						G.E_MANAGER:add_event(Event({
							trigger = 'after',
							delay = 0.1,
							func = function()
								for _, card in ipairs(polychrome_cards) do
									card:set_edition({polychrome = true}, true, false)
								end
								return {
									card = self,
									message = localize('k_chromahand')
								}
							end}))
						end
					end
			...
			if context.cardarea == G.jokers then
                if context.before then				
					if self.ability.name == "Fermata's Tremolo" and not context.blueprint then
						self.ability.extra.total = self.ability.extra.total + self.ability.extra.plus
						if math.floor(self.ability.extra.total) == self.ability.extra.total then
							message = localize('k_upgrade_ex') --cant get this message to override the damn +total message
							end
						return {
							message = localize{type='variable', key='a_chips', vars={self.ability.extra.total}},
							colour = G.C.RED,
							delay = 0.5,
							card = self
							}
					end
					if self.ability.name == 'Cicero' then
						...
						level_up_hand(context.blueprint_card or self, text, nil, self.ability.extra) --UPGRADES IT BY 5!!! OR HOWEVER MANY FUCKING TIMES I'VE PUT IT IN GAME.LUA!!!!!!!!
						update_hand_text(
							{sound = 'button', volume = 0.7, pitch = 1.1, delay = 0}, 
							{mult = 0, chips = 0, handname = '', level = ''}
						)
					end
					if self.ability.name == 'Mr. Hands' and not context.blueprint then
					return {
							message = localize('k_upgrade_ex'),
							card = self
						}
					end
					if self.ability.name == 'Number Two' and not context.blueprint then
						local twos = 0
						for i = 1, #context.scoring_hand do
						...
						self.ability.mult = (self.ability.mult or 0) + self.ability.extra
							return {
								message = localize('k_upgrade_ex'),
								colour = G.C.RED,
								card = self
                    ...
                    if self.ability.name == 'Mafia' and G.GAME.current_round.hands_played == 0 and not context.blueprint then
                    ...
                    if self.ability.name == 'Chameleon Hook' and context.scoring_name == self.ability.hook_poker_hand then
                        return {
                            card = self,
                            level_up = true,
					...
					if self.ability.name == 'Pyramid' and not context.blueprint then
						local reset = true
				...
				if self.ability.name == 'Tab' and not context.blueprint then
					self.ability.extra.gain = self.ability.extra.gain - self.ability.extra.lose
					if self.ability.extra.gain <= 0 then 
						G.E_MANAGER:add_event(Event({func = function()
							play_sound('blackfriday')
							self.T.r = -0.2
							self:juice_up(0.3, 0.4)
							...
							return {
								message = localize{type='variable', key='a_money_minus', vars={self.ability.extra.lose}},
								card = self,
								colour = G.C.RED
								}
							end
						end
						if self.ability.name == 'Pendulum' and not context.blueprint then
							local togel = self.ability.extra.toggle
							if togel == 1 then
								--xMult
								return {
				...
				--NEW JOKERS ENDS HERE
                    ...
                    if self.ability.name == 'Seltzer' and not context.blueprint then
						...
						if self.ability.name == "No, Thank You" and self.ability.x_mult > 0 then
							return {
								message = localize{type='variable', key='a_xmult', vars={self.ability.x_mult}},
								Xmult_mod = self.ability.x_mult
							}
						end
						if self.ability.name == 'White Collar Joker' and self.ability.x_chip > 0 then
							return {
								message = localize{type='variable', key='a_xchip', vars={self.ability.x_chip}},
								Xchip_mod = self.ability.x_chip
							}
						end
						if self.ability.name == 'Aglio e Olio' and (self.ability.x_chip > 0 or self.ability.x_mult > 0) then 
							return {
								message = localize{type='variable', key='a_xchip', vars={self.ability.x_chip}} .. ' | ' .. localize{type='variable', key='a_xmult', vars={self.ability.x_mult}},
								Xchip_mod = self.ability.x_chip,
								Xmult_mod = self.ability.x_mult
							}
						end
						if self.ability.name == 'Mr. Hands' and G.GAME.hands_played > 0 then
							local total_chips = self.ability.extra * G.GAME.hands_played
							return {
								message = localize{type='variable', key='a_chips', vars={total_chips}},
								chip_mod = total_chips,
							}
						end
						if (self.ability.name == 'Castor' and #find_joker('Pollux') > 0) then
							return {
								message = localize{type='variable', key='a_xchip', vars={self.ability.extra}},
								Xchip_mod = self.ability.extra
							}
						end
						if (self.ability.name == 'Pollux' and #find_joker('Castor') > 0) then
							return {
								message = localize{type='variable', key='a_xmult', vars={self.ability.extra}},
								Xmult_mod = self.ability.extra
							}
						end
						if self.ability.name == 'Number Two' and self.ability.mult > 0 then
							return {
								message = localize{type='variable',key='a_mult',vars={self.ability.mult}},
								mult_mod = self.ability.mult
							}
						end
                        if self.ability.name == 'Potato Chips' then
                            return {
						...
						if self.ability.name == 'Hoarder' and #G.consumeables.cards > 0 then
							return {
                                message = localize{type='variable',key='a_mult',vars={0 + self.ability.extra*#G.consumeables.cards}},
                                mult_mod = 0 + self.ability.extra*#G.consumeables.cards, 
                                colour = G.C.MULT
							}
						end
						if self.ability.name == 'Cobblestone Joker' and self.ability.mult > 0 then
							return {
								message = localize{type='variable',key='a_mult',vars={self.ability.mult}},
								mult_mod = self.ability.mult
							}
						end
						if self.ability.name == 'Pocket Lint' and G.GAME.current_round.current_hand.chips <= self.ability.extra.basechips then
							return {
								message = localize{type='variable',key='a_xchip',vars={self.ability.extra.Xchip}},
								Xchip_mod = self.ability.extra.Xchip,
							}
						end
                        if self.ability.name == 'Gasbag' then
                            return {
						...
						if self.ability.name == 'Polyglot Joker' then
							local effect = self.ability.extra.toggle
							if effect == 2 then
								return {
									message = localize{type='variable', key='a_mult', vars={self.ability.extra.mult}}, --mult
									mult_mod = self.ability.extra.mult
								}
							elseif effect == 3 then
								return {
									message = localize{type='variable', key='a_chips', vars={self.ability.extra.chips}}, --chips
									colour = G.C.BLUE,
									chip_mod = self.ability.extra.chips
								}
							elseif effect == 4 then
								return {
									message = localize{type='variable', key='a_xmult', vars={self.ability.extra.Xmult}}, --xMult
									Xmult_mod = self.ability.extra.Xmult
								}
							elseif effect == 5 then
								return {
									message = localize{type='variable', key='a_xchip', vars={self.ability.extra.Xchips}}, --xChips
									colour = G.C.BLUE,
									Xchip_mod = self.ability.extra.Xchips
								}
							end
						end
						if self.ability.name == 'Pendulum' then
							if self.ability.extra.Xmult == nil then self.ability.extra.Xmult = 2 end
							if self.ability.extra.Xchips == nil then self.ability.extra.Xchips = 3 end
							if self.ability.extra.toggle == nil then self.ability.extra.toggle = 1 end
							local togel = self.ability.extra.toggle --hehe togel
							if not context.blueprint then
								self.ability.extra.toggle = (self.ability.extra.toggle == 1) and 0 or 1
							end
							if togel == 1 then
								--xMult
								return {
							...
							if togel == 0 then
								--xChips
								return {
									message = localize{type='variable', key='a_xchip', vars={self.ability.extra.Xchips}},
									colour = G.C.BLUE,
									Xchip_mod = self.ability.extra.Xchips
								}
							end
						end
                        if self.ability.name == 'Sacrificial Tanto' and self.ability.x_mult > 0 then
							return {
								message = localize{type='variable',key='a_xmult',vars={self.ability.x_mult}},
								colour = G.C.RED,
								Xmult_mod = self.ability.x_mult
                        ...
                        if self.ability.name == 'Blue Collar Joker' and self.ability.x_chip > 1 then
                            return {
                                message = localize{type='variable',key='a_xchip',vars={self.ability.x_chip}},
                                Xchip_mod = self.ability.x_chip
                            }
                        end
                        if self.ability.name == 'Eye for an Eye' and self.ability.x_chip > 1 then
                            return {
                                message = localize{type='variable',key='a_xchip',vars={self.ability.x_chip}},
                                Xchip_mod = self.ability.x_chip
                            }
                        end
						if self.ability.name == 'Multi-Tool' and (self.ability.mult > 0 or self.ability.extra.chips > 0) then 
							return {
								message = localize{type='variable', key='a_chips', vars={self.ability.extra.chips}} .. ' Chips | ' .. localize{type='variable', key='a_mult', vars={self.ability.extra.mult}},
								chip_mod = self.ability.extra.chips,
								mult_mod = self.ability.mult
							}
						end
                        if self.ability.name == 'Last Resort' and #G.deck.cards == 0 then
                            return {
                                message = localize{type='variable',key='a_xmult',vars={self.ability.extra}},
                                Xmult_mod = self.ability.extra, 
                                colour = G.C.MULT
                        ...
                        if self.ability.name == "Driver's License" then
                            if (self.ability.driver_tally or 0) >= 10 then 
                                return {
                                    message = localize{type='variable',key='a_xmult',vars={self.ability.extra}},
                                    Xmult_mod = self.ability.extra
                                }
...
function Card:update(dt)
    ...
    if G.STAGE == G.STAGES.RUN then
		...
		--Mezzetino totaling (just copying the code from function calculate dollar bonus lol)
		if self.ability.name == 'Mezzetino' then
			self.ability.extra.total = 0
			for i = 1, #G.jokers.cards do
				if G.jokers.cards[i] ~= self and G.jokers.cards[i].ability.set == 'Joker' then
		...
		if self.ability.name == "Moolah" then 
			self.ability.gold_tally = 0
			for k, v in pairs(G.playing_cards) do
				if v.config.center == G.P_CENTERS.m_gold then 
					self.ability.gold_tally = self.ability.gold_tally + 1 
				end
			end
		end
        ...
        if self.ability.name == "Joker Stencil" then 
            ...
            for i = 1, #G.jokers.cards do
                if G.jokers.cards[i].ability.name == 'Joker Stencil' then self.ability.x_mult = self.ability.x_mult + 2 end
            end
        end
        if self.ability.name == 'The Wheel of Fortune' then
        ...
        elseif self.sprite_facing == 'front' then 
            ...
            if self.edition or self.seal or self.ability.eternal or self.ability.rental or self.ability.perishable or self.sticker or ((self.sticker_run and self.sticker_run ~= 'NONE') and G.SETTINGS.run_stake_stickers) or (self.ability.set == 'Spectral') or self.debuff or self.greyed or (self.ability.name == 'The Soul') or (self.ability.set == 'Voucher') or (self.ability.set == 'Booster') or self.config.center.soul_pos or self.config.center.demo then
                ...
                if self.config.center.soul_pos and (self.config.center.discovered or self.bypass_discovery_center) then
                    ...
                    if self.ability.name == 'Hologram' or self.ability.name == 'Spirit Box' then
...
function Card:load(cardTable, other_card)
    ...
    elseif self.config.center.name == "Photograph" then 
        ...
        self.T.w = W*scale
	elseif self.config.center.name == "Last Resort" then 
		self.T.h = H*scale/1.2*scale
		self.T.w = W*scale
    elseif self.config.center.name == "Square Joker" then
        ...
        self.T.h = H*scale
        self.T.w = W*scale
    elseif self.config.center.name == "Crumpled Lucky Card" then
        H = W 
        self.T.h = H*scale
        self.T.w = W*scale
