...
function card_eval_status_text(card, eval_type, amt, percent, dir, extra)
	...
	sound = extra.edition and 
		'foil2' or extra.mult_mod and
		'multhit1' or extra.Xmult_mod and 
		'multhit2' or extra.Xchip_mod and
		'multhit3' or 'generic1'
...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    if not forced_key and soulable and (not G.GAME.banned_keys['c_soul']) then
        ...
        not (G.GAME.used_jokers['c_soul'] and not next(find_joker("Showman")))  then
            if pseudorandom('soul_'.._type..G.GAME.round_resets.ante) > 0.995 then
                forced_key = 'c_soul'
            end
        end
        if (_type == 'Planet' or _type == 'Spectral') and
        not (G.GAME.used_jokers['c_black_hole'] and not next(find_joker("Showman")))  then 
            if pseudorandom('soul_'.._type..G.GAME.round_resets.ante) > 0.9975 then
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Joker' then
        ...
        elseif _c.name == 'Void' then info_queue[#info_queue+1] = {key = 'tag_negative', set = 'Tag'}
        elseif _c.name == 'Black Friday' then info_queue[#info_queue+1] = {key = 'tag_coupon', set = 'Tag'}
        elseif _c.name == 'Framework' then info_queue[#info_queue+1] = {key = 'j_blueprint', set = 'Joker'}
        elseif _c.name == 'Encore' then info_queue[#info_queue+1] = {key = 'c_fool', set = 'Tarot'} ; info_queue[#info_queue+1] = {key = 'e_negative_consumable', set = 'Edition', config = {extra = 1}}
        elseif _c.name == 'Mafia' then info_queue[#info_queue+1] = {key = 'red_seal', set = 'Other'}
        elseif _c.name == 'Cobblestone Joker' then info_queue[#info_queue+1] = G.P_CENTERS.m_stone
        elseif _c.name == 'Ooga Booga' then info_queue[#info_queue+1] = G.P_CENTERS.m_stone
        elseif _c.name == 'Miner' then info_queue[#info_queue+1] = G.P_CENTERS.m_stone
        elseif _c.name == 'Disciple' then info_queue[#info_queue+1] = G.P_CENTERS.m_bonus
        elseif _c.name == 'Moolah' then info_queue[#info_queue+1] = G.P_CENTERS.m_gold
        elseif _c.name == 'Potato Chips' then info_queue[#info_queue+1] = G.P_CENTERS.m_bonus
        elseif _c.name == 'Chromahand' then info_queue[#info_queue+1] = G.P_CENTERS.e_polychrome
        elseif _c.name == "No, Thank You" then info_queue[#info_queue+1] = G.P_CENTERS.m_steel ; info_queue[#info_queue+1] = G.P_CENTERS.m_gold
        elseif _c.name == "Oh, Lord" then info_queue[#info_queue+1] = {key = 'c_emperor', set = 'Tarot'} ; info_queue[#info_queue+1] = {key = 'purple_seal', set = 'Other'} 
        elseif _c.name == 'Lottery Card' then info_queue[#info_queue+1] = {key = 'c_soul', set = 'Spectral'}
        elseif _c.name == 'Big Money' then info_queue[#info_queue+1] = G.P_CENTERS.m_lucky
        elseif _c.name == 'Castor' then info_queue[#info_queue+1] = {key = 'j_pollux', set = 'Joker'}
        elseif _c.name == 'Pollux' then info_queue[#info_queue+1] = {key = 'j_castor', set = 'Joker'}
        elseif _c.name == 'Russian Roulette' then info_queue[#info_queue+1] = G.P_CENTERS.e_negative
		elseif _c.name == 'Scratch Card' then info_queue[#info_queue+1] = {key = 'c_soul', set = 'Spectral'}
    ...
    elseif _c.set == 'Spectral' then 
		...
		elseif _c.name == 'Summon' then
			...
			local colour = (not summon_c or summon_c.name == 'Summon' or nono_spectral[G.GAME.last_spectral]) and G.C.RED or G.C.GREEN
				main_end = {
		...
		if summon_c and summon_c.name ~= 'Summon' and not nono_spectral	[G.GAME.last_spectral] then
