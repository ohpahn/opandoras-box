            ...
            if scoring_hand[i].debuff then
                ...
                for j=1,#reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        --If x_chip added, do x_chip add event and mult the mult to the total
                        if effects[ii].x_chip then 
                            if effects[ii].card then juice_card(effects[ii].card) end
                            hand_chips = mod_chips(hand_chips*effects[ii].x_chip)
                            update_hand_text({delay = 0}, {chips = hand_chips})
                            card_eval_status_text(scoring_hand[i], 'x_chip', effects[ii].x_chip, percent)
                        end
                ...
                while j <= #reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        if effects[ii].x_chip then
                            ...
                            card_eval_status_text(G.hand.cards[i], 'x_chip', effects[ii].x_chip, percent)
                        end
