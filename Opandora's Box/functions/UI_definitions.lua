...
function create_UIBox_debug_tools()
  ...
  G.FUNCS.DT_x_chip = function() if G.STAGE == G.STAGES.RUN then update_hand_text({delay = 0}, {chips = 10*G.GAME.current_round.current_hand.chips}); play_sound('multhit3') end end
...
function G.UIDEF.settings_tab(tab)
  if tab == 'Game' then
    return {n=G.UIT.ROOT, config={align = "cm", padding = 0.05, colour = G.C.CLEAR}, nodes={
      create_option_cycle({label = localize('b_set_gamespeed'),scale = 0.8, options = {0.5, 1, 2, 4, 6, 10, 20, 50, 100}, opt_callback = 'change_gamespeed', 
	  
		--first value is the game speed, second value is the switch order
        current_option = (
										...
										G.SETTINGS.GAMESPEED == 6 		and 5 or 
										G.SETTINGS.GAMESPEED == 10 	and 6 or
										G.SETTINGS.GAMESPEED == 20 	and 7 or
										G.SETTINGS.GAMESPEED == 50		and 8 or
										G.SETTINGS.GAMESPEED == 100 	and 9
									)
      }),
...
function create_UIBox_your_collection_spectrals()
  ...
  for i = 1, math.ceil(#G.P_CENTER_POOLS.Tarot/9) do
...
function create_UIBox_your_collection_tags()
  ...
  for k, v in ipairs(tag_tab) do
    ...
    tag_matrix[math.ceil((k-1)/7+0.001)][1+((k-1)%7)] = {n=G.UIT.C, config={align = "cm", padding = 0.1}, nodes={
      temp_tag_ui,
    }}
