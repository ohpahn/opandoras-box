...
function Tag:apply_to_run(_context)
    if not self.triggered and self.config.type == _context.type then
        ...
        elseif _context.type == 'round_start_bonus' then 
			...
			if self.name == 'Focus Tag' then
				self:yep('+', G.C.BLUE, function() 
				if G.hand and G.hand.config then --modify highlighted_limit for G.hand and calls it from cardarea.lua
					G.hand.config.highlighted_limit = (G.hand.config.highlighted_limit or 5) + 1 --+1 to highlighted limit
				end
			return true
		end)
	return true
        ...
        elseif _context.type == 'shop_start' then
            if self.name == 'D6 Tag' and not G.GAME.shop_d6ed then
                ...
                self:yep('+', G.C.GREEN,function() 
                    G.GAME.round_resets.temp_reroll_cost = -1
...
function Tag:get_uibox_table(tag_sprite)
    ...
    elseif name_to_check == 'Focus Tag' then
    end
